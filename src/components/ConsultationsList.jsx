      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          <h2 className="font-display font-semibold text-lg text-ram-secondary">
            Mes consultations tarifaires
          </h2>
          <p className="text-xs text-ink-muted mt-1">
            Demandes de prix initiées par la Direction Achats RAM
          </p>
        </div>
      </div>
        ...
        {consultations.map((c, i) => (
          ...
          >
            ...
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-[11px] text-ink-muted font-mono">{c.id}</span>
            </div>
            ...
            <div className="pt-4 border-t border-dashed border-gray-200 grid grid-cols-3 gap-2">
              <Stat value={c.invitedHotels} label="Hôtels" />
              <Stat value={c.submissions} label="Offres" />
              <Stat
                value={c.bestPrice ? c.bestPrice : '—'}
                label="Best (MAD)"
                highlight={!!c.bestPrice}
              />
            </div>
...
function StatusBadge({ status }) {
  const config = {
    ...
    pending: { label: 'À soumettre', cls: 'bg-orange-50 text-orange-700 border-orange-200' },
    closed: { label: 'Clôturée', cls: 'bg-gray-100 text-gray-600 border-gray-200' },
  }
...
function Stat({ value, label, highlight }) {
  return (
    <div>
      <div className={`font-display font-bold text-lg ${highlight ? 'text-ram-primary' : 'text-ram-secondary'}`}>
        {value}
      </div>
